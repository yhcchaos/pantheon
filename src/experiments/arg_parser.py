...
def parse_test_shared(local, remote, config_args):
    for mode in [local, remote]:
        ...
        mode.add_argument(
            '--actor_id', type=int, default=0,
            help='actor id who run this envirment'
        )
        mode.add_argument(
            '--episode_id', type=int, default=0,
            help='episode id'
        )
        mode.add_argument(
            '--do_log', action='store_true', 
            help='do we log the running infomation generated by mm-link and mm-tunnel?'
        )
        if config_args.config_file is None:
